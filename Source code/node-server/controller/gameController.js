...
const find_winners = Symbol('find_winners'),
	  create_new_game = Symbol('create_new_game'),
	  sys_game_schedule = Symbol('sys_game_schedule');
	...
	[sys_game_schedule]() {
		...
		.then((active_game)=>{
			if(active_game!=null){
				...
				if(isDeadline){
					this[find_winners](active_game.END_DATE)
					.then((data)=>{
...
//schedule system game
	sys_game_controller_running(){
		schedule.scheduleJob('2 0 0 * * *', ()=>{
			this[sys_game_schedule]();
		});
	};

//schedule yn games
	yn_game_controller_running(){
